...
%     nearest_points = pc(index(1:n_neighbour),:);
    nearest_points = pc(dis<0.5,:);
    n_neighbour = size(nearest_points,1);
    n_samples = 50;
    
    max_inliers = 0;
    inliers = nearest_points;
    [n,p] = affine_fit(nearest_points);
    
    theta = n;
    alpha = p*n;
    
    index = find(sqrt(sum(bsxfun(@minus,inliers,p).^2,2))<0.5);
...
function [theta, alpha, minError, inliers] = ObtainPlanePointOld(pc,point)
